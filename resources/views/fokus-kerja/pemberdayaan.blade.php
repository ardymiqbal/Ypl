...
@section('title','Pemberdayaan Masyarakat Lokal')
...
<section class="bg-white rounded-2xl shadow p-4 sm:p-6 md:p-8">
  ...
  <h2 class="mt-1 text-blue-700 font-semibold text-balance text-[clamp(16px,4.8vw,22px)] md:text-3xl">
    Membangun Bersama, Bertumbuh Bersama
  </h2>

  {{-- Foto utama: tidak terlalu tinggi di HP, tetap besar di desktop --}}
  ...
  {{-- Deskripsi: ukuran font nyaman di HP & desktop --}}
  <div class="mt-5 font-sans font-medium text-gray-800 text-pretty
              text-[15px] leading-7 md:text-[18px] md:leading-8">
    <p>
      Pemberdayaan masyarakat adalah proses membangun kesadaran, kapasitas, dan kemandirian warga untuk terlibat aktif dalam mengelola sumber daya serta menyelesaikan persoalan yang mereka hadapi. Di tengah tantangan lingkungan, sosial, dan ekonomi, pendekatan ini menjadi strategi penting untuk menciptakan perubahan yang berkelanjutan.
    </p>
    <p class="mt-3">
      Melalui pelatihan, pendampingan, dan akses informasi, masyarakat tidak hanya diberi bantuan, tetapi juga diberi kesempatan untuk berkembang dan menentukan arah hidupnya sendiri. Misalnya, fokus-kerja pengelolaan sampah berbasis warga, pertanian organik, hingga usaha kreatif ramah lingkungan telah terbukti meningkatkan kesejahteraan sekaligus menjaga kelestarian lingkungan.
    </p>
    <p class="mt-3">
      Inti dari pemberdayaan bukan hanya soal fokus-kerja, tetapi soal membangun rasa percaya diri dan kepemilikan. Ketika masyarakat merasa memiliki peran dan mampu membuat perubahan, maka solusi yang tercipta akan lebih tepat sasaran dan berkelanjutan.
    </p>
  </div>
</section>
  ...
  @if($latestArticles->count())
    <div class="mt-3 grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
      @foreach($latestArticles as $a)
        @php
          $thumb = Str::startsWith($a->thumbnail, 'http')
            ? $a->thumbnail
            : route('articles.thumb'.$a->thumbnail);
        @endphp
