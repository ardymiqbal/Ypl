...
<section class="bg-white rounded-2xl shadow p-4 sm:p-6 md:p-8">
  ...
  <h2 class="mt-1 text-blue-700 font-semibold text-balance
             text-[clamp(16px,4.8vw,22px)] md:text-3xl">
    Kolaborasi dan Jejaring sebagai Strategi Penguatan Gerakan Lingkungan
  </h2>

  {{-- HERO: tidak terlalu tinggi di HP, tetap besar di desktop --}}
  <div class="mt-6 overflow-hidden rounded-2xl ring-1 ring-black/5 shadow">
    <img src="{{ $hero }}" alt="Kolaborasi & Jejaring"
         class="w-full h-56 sm:h-64 md:h-[28rem] lg:h-[34rem] xl:h-[40rem] object-cover object-center">
  </div>

  {{-- Narasi/deskripsi: nyaman dibaca di HP & desktop --}}
  <div class="mt-5 font-sans font-medium text-gray-800 text-pretty
              text-[15px] leading-7 md:text-[18px] md:leading-8">
    <p>
      Dalam upaya mewujudkan pembangunan lingkungan yang berkelanjutan, kolaborasi dan jejaring merupakan pilar strategis
      yang tidak dapat diabaikan. Kompleksitas persoalan lingkungan membutuhkan pendekatan lintas sektor, lintas disiplin,
      serta keterlibatan aktif dari berbagai pemangku kepentingan.
    </p>
    <p class="mt-3">
      Yayasan kami secara konsisten membangun kemitraan dengan komunitas lokal, institusi pemerintahan, akademisi, sektor
      swasta, dan organisasi masyarakat sipil. Kolaborasi ini bertujuan untuk memperkuat kapasitas bersama, mengoptimalkan
      sumber daya yang tersedia, serta menciptakan solusi yang kontekstual dan berkelanjutan.
    </p>
    <p class="mt-3">
      Melalui pengembangan jejaring yang inklusif dan partisipatif, kami mendorong terbentuknya ekosistem kolaboratif yang
      mampu memperluas dampak fokus-kerja, meningkatkan efektivitas intervensi, serta mempercepat tercapainya tujuan pelestarian
      lingkungan.
    </p>
  </div>

  {{-- TIGA FOTO: besar di desktop, proporsional di HP --}}
    ...
    @foreach($mid as $src)
      <button type="button"
              class="js-open-media block overflow-hidden rounded-xl bg-white ring-1 ring-black/5 shadow hover:shadow-lg transition
                     max-w-[520px] mx-auto sm:max-w-none sm:mx-0"
              data-title="Kegiatan Kolaborasi"
              data-type="image"
              data-src="{{ $src }}">
        <img src="{{ $src }}" alt="Kegiatan Kolaborasi"
      ...
      @foreach($latestArticles as $a)
        ...
        <article class="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden ring-1 ring-black/5">
          <a href="{{ route('articles.thumb',$a->slug) }}">
            {{-- Lebih pendek di HP, 16:9 di desktop --}}
          ...
          <div class="p-3 sm:p-4">
            ...
            <p class="mt-1 text-[11px] sm:text-xs text-gray-500">
              {{ $a->author ?? 'Redaksi' }} · {{ optional($a->created_at)->format('d M Y') }}
            </p>
          </div>
        </article>
      @endforeach
